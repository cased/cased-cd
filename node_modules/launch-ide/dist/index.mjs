...
var N = (t, e, n) => e in t ? W(t, e, { enumerable: !0, configurable: !0, writable: !0, value: n }) : t[e] = n, v = (t, e) => {
  for (var n in e || (e = {}))
    H.call(e, n) && N(t, n, e[n]);
  if (B)
    for (var n of B(e))
      V.call(e, n) && N(t, n, e[n]);
  return t;
}, _ = (t, e) => $(t, F(e));
import I from "fs";
import y from "path";
...
function q(t, e, n, o) {
  ...
  if (typeof o == "string")
    s = o.replace(m, t).replace(u, e.toString()).replace(M, n.toString());
...
function K(t) {
  const {
    ...
    openWindowParams: i,
    pathFormat: c
  } = t, h = { editorBasename: Y(e), openWindowParams: i, workspace: r }, l = ee(h) || "{file}";
  return q(n, o, s, c || l);
}
function Y(t) {
  ...
  for (let s = 0; s < o.length; s++)
    if ((P[n][o[s]] || []).some((i) => t.endsWith(i))) {
      e = o[s];
      break;
    }
...
function ne(t, e, n) {
  ...
  try {
    let r;
    const i = process.platform, c = process.platform === "win32", f = te[i], h = R[i];
    L(c);
    const l = E.execSync(f, { encoding: "utf-8" }), S = Object.keys(h), w = l.split(c ? "\r\n" : "\n").map((O) => O.trim());
    for (let O = 0; O < S.length; O++) {
      const a = S[O];
      let C = "", x = "";
      if (c) {
        const p = w.find(
          (g) => y.basename(g).toLowerCase() === a.toLowerCase()
        );
        p && (x = y.basename(p), C = p);
      } else if (i === "darwin") {
        const p = w.find(
          (g) => g.toLowerCase().endsWith(a.toLowerCase())
        );
        if (p) {
          const g = p.replace(a, ""), T = h[a];
          x = a, T.includes("/") ? C = "".concat(g).concat(T) : C = T;
        }
      } else
        l.indexOf(a) !== -1 && (x = a, C = h[a]);
      if (x && C) {
...
function oe() {
  ...
  try {
    return re(process.pid, t, e);
  } catch (n) {
    return console.log(d.red("Error while getting editor by PID:"), n), null;
  }
}
function re(t, e, n) {
  let o = 0;
...
function ae(t) {
  try {
    const n = E.execSync("ps -p ".concat(t, " -o ppid=,comm="), {
      encoding: "utf8"
    }).trim().split("\n");
    return n.length ? ie(n[0].trim()) : null;
  } catch (e) {
    return null;
  }
}
function ce(t) {
  try {
    L(!0);
    const e = 'powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_Process -Filter "ProcessId='.concat(t, '" | Select-Object ParentProcessId, ExecutablePath | ConvertTo-Csv -NoTypeInformation'), o = E.execSync(e, {
      encoding: "utf8"
...
function D(t, e) {
  console.log(
    d.red("Could not open " + y.basename(t) + " in the editor.")
  ), e && (e[e.length - 1] !== "." && (e += "."), console.log(
    d.red("The editor process exited with an error: " + e)
  )), console.log(
    "To set up the editor integration, add something like " + d.cyan("CODE_EDITOR=code") + " to the " + d.green(".env.local") + " file in your project folder, or add " + d.green('editor: "code"') + " to CodeInspectorPlugin config, and then restart the development server. Learn more: " + d.green("https://goo.gl/MMTaZt")
  );
}
let b = null;
function me(t) {
...
function be(t) {
  ...
  let [l, ...S] = ne(s, f, h);
  const w = de(f || "") || i;
  if (!l || l.toLowerCase() === "none") {
    typeof c == "function" ? c(e, "Failed to recognize IDE automatically") : console.log(
      "Failed to recognize IDE automatically, add something like " + d.cyan("CODE_EDITOR=code") + " to the " + d.green(".env.local") + " file in your project folder, or add " + d.green('editor: "code"') + " to CodeInspectorPlugin config, and then restart the development server. Learn more: " + d.green("https://goo.gl/MMTaZt")
    );
    return;
  }
  ...
  if (n ? S = S.concat(
    K({
      ...
      colNumber: o,
      workspace: O,
      openWindowParams: me(r),
      pathFormat: w
    })
  ) : S.push(e), b && le(l) && b.kill("SIGKILL"), process.platform === "win32") {
    const a = (p) => p.replace(/([&|<>,;=^])/g, "^$1"), C = (p) => p.includes("^") ? '^"'.concat(p, '^"') : p.includes(" ") ? '"'.concat(p, '"') : p, x = [l, ...S.map(a)].map(C).join(" ");
    b = E.exec(x, {
      stdio: "ignore",
      // @ts-ignore
      shell: !0,
  ...
  b.on("exit", function(a) {
    b = null, a && (typeof c == "function" ? c(e, "(code " + a + ")") : D(e, "(code " + a + ")"));
  }), b.on("error", function(a) {
    typeof c == "function" ? c(e, a.message) : D(e, a.message);
  });
}
export {
  q as formatOpenPath,
