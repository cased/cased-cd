...
import { IconCircleCheck, IconCircleWarning, IconClock3, IconCircleInfo, IconChevronRight, IconChevronDown } from 'obra-icons-react'
...
function ResourceNode({ data }: { data: ResourceNodeData }) {
  ...
  return (
    <div className="relative">
      ...
      <Handle type="target" position={Position.Left} className="!bg-neutral-400 dark:!bg-neutral-600" />
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium text-neutral-500 dark:text-neutral-500 uppercase">{resource.kind}</span>
        </div>
        <div className="font-medium text-black dark:text-white text-sm mb-1 truncate">{resource.name}</div>
        {resource.namespace && (
          <div className="text-xs text-neutral-600 dark:text-neutral-400">ns: {resource.namespace}</div>
        )}
        <div className="mt-2">
          <Badge
      ...
      <Handle type="source" position={Position.Right} className="!bg-neutral-400 dark:!bg-neutral-600" />
    </div>
  )
}
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-semibold text-black dark:text-white">{data.kind}</span>
        ...
        <div className="flex gap-2 flex-wrap mt-2">
          {Object.entries(data.healthSummary).map(([status, count]) => {
            const health = healthIcons[status as keyof typeof healthIcons] || healthIcons.Unknown
            const HealthIcon = health.icon
...
export function ResourceTree({ resources, onResourceClick }: ResourceTreeProps) {
  ...
  const { nodes: initialNodes, edges: initialEdges } = useMemo(() => {
    ...
    resources.forEach(resource => {
      ...
      resource.parentRefs?.forEach(parent => {
        ...
        if (expandedGroups.has(resource.kind) && expandedGroups.has(parent.kind)) {
          edges.push({
        ...
        // If source is collapsed but target is expanded, connect group to node
        else if (!expandedGroups.has(parent.kind) && expandedGroups.has(resource.kind)) {
          ...
          if (!edges.find(e => e.id === edgeId)) {
            edges.push({
              ...
              target: targetNodeId,
              type: 'smoothstep',
              style: { stroke: '#8c8c8c', strokeWidth: 1.5, strokeDasharray: '5,5' },
            })
          }
        }
        ...
        // If both collapsed, connect groups
          ...
          if (!edges.find(e => e.id === edgeId)) {
            edges.push({
              ...
              target: targetGroupId,
              type: 'smoothstep',
              style: { stroke: '#737373', strokeWidth: 2, strokeDasharray: '5,5' },
            })
          }
        }
      })
  ...
  return (
    <div className="h-[650px] rounded-lg border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 overflow-hidden">
      ...
      >
        <Background gap={16} className="[&_path]:stroke-neutral-300 dark:[&_path]:stroke-neutral-700" />
