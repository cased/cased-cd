...
**Note:** The enterprise manifest includes a PVC for audit logs. If your cluster doesn't have a default storage class:
- Install local-path-provisioner (Talos/k3d/kind): `kubectl apply -f https://raw.githubusercontent.com/rancher/local-path-provisioner/v0.0.24/deploy/local-path-storage.yaml`
- Or use Helm with `--set enterprise.auditTrail.enabled=false` to disable audit logs
